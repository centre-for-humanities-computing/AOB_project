...
def get_nlp(lang: str):
    ...
    if lang == "english":
        ...
        try:
            ...
            raise OSError(
                "ko_core_news_sm not downloaded, run python3 -m spacy download ko_core_news_sm"
            ) from e
        ...
        try:
            ...
            raise OSError(
                "de_core_news_sm not downloaded, run python3 -m spacy download de_core_news_sm"
            ) from e
        ...
        try:
            ...
            raise OSError(
                "it_core_news_sm not downloaded, run python3 -m spacy download it_core_news_sm"
            ) from e
...
def text_readability(text: str, language):
    
    if language == "korean" or language == 'german':
        try:
            ...
            raise OSError(
                "error w korean readability"
            ) from e

        return flesch_grade, flesch_ease, smog, ari
